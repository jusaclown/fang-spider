...
class FangtianxiaSpider(scrapy.Spider):
        ...
        for tr in trs:
            ...
            for city_link in city_links:
                ...
                if 'bj' in url_module[0]:
                    ...
                    # 构建新房的url链接
                    newshouse_url = url_module[0] + '.newhouse.' + url_module[1] + 'house/s'
                    # 构建二手房的url链接
                    ...
                    #print("二手房", esf_url)                
                yield scrapy.Request(url=newshouse_url, callback=self.parse_newhouse,
    ...
    def parse_newhouse(self, response):
        ...
        for li in lis:
            ...
            sale = response.xpath(".//div[@class='fangyuan pr']/span/text()").get() 
            origin_url = response.xpath(".//div[@class='nlcd_name']/a/@href").get()
    ...
    def parse_esf(self, response):
        province, city = response.meta.get('info')
        dls = response.xpath("//div[contains(@class, 'shop_list')]/dl[@dataflag!='bgcomare']")
        for dl in dls:
        ...
        for next_url in next_urls:    
            if next_url.xpath('./text()').get() == "下一页":
                ...
                break
        if next_page_url:
            yield scrapy.Request(url=response.urljoin(next_page_url), callback=self.parse_esf,
                                 meta={"info":(province,city)})